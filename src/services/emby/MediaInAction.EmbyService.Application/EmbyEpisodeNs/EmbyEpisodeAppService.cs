...
public class EmbyEpisodeAppService : EmbyServiceAppService, IEmbyEpisodeAppService
{
    ...
    {
        var embyEpisodeAliasDtos = GetEpisodeAliasTuple(input.EpisodeAliases);

        var dbEpisode = await _embyEpisodeRepository.GetByEmbyShowSeasonEpisodeAsync(
            input.ShowName, input.SeasonNum, input.EpisodeNum);
        
        var newEpisode = await _embyEpisodeManager.CreateAsync(
            input.ShowName,
            input.SeasonNum,
            input.EpisodeNum
        );
